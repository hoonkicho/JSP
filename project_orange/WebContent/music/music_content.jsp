		...
		<div class="container">
			<div class="raw">
				...
				<table class="table">
					...
					<ul class="nav navbar-nav navbar-right">
						<li class="dropdown">
							...
							<ul class="dropdown-menu">
								<li><a href="otherlist_list.korea?idx=${vo.memberidx}">${vo.name}님의
										PlayList</a></li>
								<li><a href="../music/other_list.korea?idx=${vo.memberidx }">${vo.name}님의
										Post</a></li>
								<c:if test="${ sessionScope.id ne vo.memberid}">
									<!-- 나에게는 친구요청과 메세지를 보낼 수 없다. -->
									<li><a href="../sns/id_list.korea?idsearch=${vo.memberid }">${vo.name}님께
											친구요청 or 메세지</a></li>
								</c:if>
							</ul></li>
					</ul>
					...
					<tr>
						<td><img src="../musicfile/${ vo.m_image }" width="50%"
							class="col-lg-4" alt="Cinque Terre"> <br>
						<h4>${ vo.m_content }</h4>
							<br>

							<h5>${ vo.m_musicfile }</h5> <audio
								src="../musicfile/${ vo.m_musicfile }" controls></audio><br>
							Like: ${ vo.m_goodpoint} <input type="button"
							class="btn btn-warning btn-sm" value="like"
							onclick="goodpoint('${vo.idx}','${vo.name }');"> <input
							type="button" class="btn btn-warning btn-sm" value="addList"
							onclick="addListf('${vo.idx}','${sessionScope.idx}');"> <c:set
								var="result" value="${vo.memberidx}" /> <c:if
							...
							</c:if> <c:if test="${ sessionScope.authority eq 1 }">
								<!-- 관리자는 무조건 삭제버튼 -->
								<input type="button" class="btn btn-warning btn-sm"
									value="delete" onclick="deletef('${vo.idx}');">
							</c:if></td>
				...
				<table align="center" width="600" style="border-collapse: collapse">
					...
					<tr>
						...
						<input
							type="button" class="btn btn-warning btn-sm" value="처음"
							...
							<c:if test="${paging.currentPage >= paging.totalPage}">
								<input type="button" class="btn btn-warning btn-sm" value="다음"
									disabled="disabled">
							</c:if><input type="button" class="btn btn-warning btn-sm" value="끝"
